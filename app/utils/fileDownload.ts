...
export class FileDownloadManager {
  ...
  static async downloadTemplate(templateId: number): Promise<boolean> {
    try {
      ...
      return await this.saveBase64File(content_base64, filename || `template_${templateId}.csv`);
    } catch (error) {
  ...
  static async downloadSample(templateId: number): Promise<boolean> {
    try {
      ...
      const { content_base64, filename } = response.data;
      return await this.saveBase64File(content_base64, filename || `sample_${templateId}.csv`);
    } catch (error) {
  ...
  static async downloadInstructions(templateId: number): Promise<boolean> {
    try {
      ...
      const response = await ScheduleImportNewAPI.downloadInstructions(templateId);
      ...
      const { content_base64, filename } = response.data;
      return await this.saveBase64File(content_base64, filename || `instructions_${templateId}.pdf`);
    } catch (error) {
  ...
  private static async saveBase64File(base64Content: string, fileName: string): Promise<boolean> {
    try {
  ...
  static async downloadAndSaveFileFromUrl(url: string, fileName: string): Promise<boolean> {
    try {
      const hasPermission = await this.requestStoragePermission();
      if (!hasPermission) {
        Alert.alert('Error', 'Storage permission is required to download files');
      ...
      if (downloadResult.status === 200) {
        ...
        if (await Sharing.isAvailableAsync()) {
          ...
          Alert.alert('File Downloaded', `File saved to: ${downloadResult.uri}`);
        }
        return true;
      } else {
